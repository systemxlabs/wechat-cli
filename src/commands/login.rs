...
pub async fn login(options: LoginOptions) -> Result<String> {
    ...
    loop {
        ...
        let status = status_resp["data"]["status"]
            .as_str()
            .or_else(|| status_resp["status"].as_str())
            .unwrap_or("unknown");
        ...
        match status {
            ...
            "confirmed" => {
                let data = status_resp.get("data").unwrap_or(&status_resp);
                ...
                let user_id = data["ilink_user_id"].as_str().context(LoginFailedSnafu {
